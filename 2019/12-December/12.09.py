# TWITTER
"""
    SOLVED -- TO BE CHECKED
    Given a binary tree and an integer k, filter the binary tree such that its leaves don't contain 
    the value k. Here are the rules:
...
def filter(tree, k):
    # Space: O(n)    Time:O(n) worst case being when all nodes are k
